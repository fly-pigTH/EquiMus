...
# 初始稳定点： pi/2, 0
StaticForce = get_static_func(math.pi/2, 0)
F1 = StaticForce[0]
F2 = StaticForce[1]
...
if viewer_flag:
  with mujoco.viewer.launch_passive(m, d) as viewer:
    ...
    try:
      while (time.time() if viewer_flag else d.time) - start < 25: # viewer.is_running() and
        ...
        if time_now - start > 0 and time_now - start < 10: # 稳定时间
          d.ctrl[:2] = F1
          d.ctrl[2:] = F2
          # record = True

        if 10 < time_now - start < 20: # 稳定时间
