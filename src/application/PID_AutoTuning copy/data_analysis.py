...
exp_log_loaded = []
data_array = np.load(CURRENT_DIR / "data" / "experiment_data.npy")
parameters_df = pd.read_csv(CURRENT_DIR / "data" / "parameters.csv")
...
def plot_pid_iterations_by_color(exp_log, duration_steady, duration_experiment):
    ...
    # high resolution
    plt.tight_layout()
    plt.savefig(CURRENT_DIR / "figure" / "joint_trajectory.png", dpi=300, bbox_inches='tight')  # Save as high-res PNG
    plt.show()
